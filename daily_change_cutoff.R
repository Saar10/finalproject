...
ibex_count_gain <- ibex_prices %>%
  filter(ibex_percent_change > 0.01) %>%
...
total_transactions_gain <- investors %>%
  group_by(investor_id) %>%
  filter(daily_position_change != 0 & ibex_percent_change > 0.01) %>%
...
# Calculate proportion of buying following 1% gain in IBEX35
...
# Calculate proportion of selling following 1% drop in IBEX35
...
# Calculate proportion of buying following 1% drop in IBEX35
...
# Calculate proportion of selling following 1% gain in IBEX35
...
uncorrelated_buy_loss <- uncorrelated_buy_loss %>%
  mutate(proportion = buy_loss / ibex_gain_total)
# Add total transactions per investor column
uncorrelated_buy_loss <- uncorrelated_buy_loss %>%
  inner_join(total_transactions, by = "investor_id") %>%
  inner_join(total_transactions_loss, by = "investor_id")
...
uncorrelated_sell_gain <- uncorrelated_sell_gain %>%
  mutate(proportion = sell_gain / ibex_loss_total)
# Add total transactions per investor column
uncorrelated_sell_gain <- uncorrelated_sell_gain %>%
  inner_join(total_transactions, by = "investor_id") %>%
  inner_join(total_transactions_gain, by = "investor_id")
...
# Export
write.csv(correlated_buy_gain, file = "correlated_buy_gain_cutoff.csv")
write.csv(correlated_buy_loss, file = "correlated_buy_loss_cutoff.csv")
write.csv(correlated_sell_gain, file = "correlated_sell_gain_cutoff.csv")
write.csv(correlated_sell_loss, file = "correlated_sell_loss_cutoff.csv")
write.csv(uncorrelated_buy_gain, file = "uncorrelated_buy_gain_cutoff.csv")
write.csv(uncorrelated_buy_loss, file = "uncorrelated_buy_loss_cutoff.csv")
write.csv(uncorrelated_sell_gain, file = "uncorrelated_sell_gain_cutoff.csv")
write.csv(uncorrelated_sell_loss, file = "uncorrelated_sell_loss_cutoff.csv")