...
correlated_buy_gain <- correlated_buy_gain %>%
  mutate(proportion = buy_gain / 285)
...
correlated_sell_loss <- correlated_sell_loss %>%
  mutate(proportion = sell_loss / 314)
# Add total transactions per investor column
correlated_sell_loss <- correlated_sell_loss %>%
  inner_join(total_transactions, by = "investor_id") %>%
...
correlated_buy_loss <- correlated_buy_loss %>%
  mutate(proportion = buy_loss / 314)
# Add total transactions per investor column
correlated_buy_loss <- correlated_buy_loss %>%
  inner_join(total_transactions, by = "investor_id") %>%
  inner_join(total_transactions_loss, by = "investor_id")
...
correlated_sell_gain <- correlated_sell_gain %>%
  mutate(proportion = sell_gain / 285)
# Add total transactions per investor column
correlated_sell_gain <- correlated_sell_gain %>%
  inner_join(total_transactions, by = "investor_id") %>%
  inner_join(total_transactions_gain, by = "investor_id")
...
# Export
write.csv(correlated_buy_gain, file = "correlated_buy_gain_dayt_cutoff.csv")
write.csv(correlated_buy_loss, file = "correlated_buy_loss_dayt_cutoff.csv")
write.csv(correlated_sell_gain, file = "correlated_sell_gain_dayt_cutoff.csv")
write.csv(correlated_sell_loss, file = "correlated_sell_loss_dayt_cutoff.csv")
write.csv(uncorrelated_buy_gain, file = "uncorrelated_buy_gain_dayt_cutoff.csv")
write.csv(uncorrelated_buy_loss, file = "uncorrelated_buy_loss_dayt_cutoff.csv")
write.csv(uncorrelated_sell_gain, file = "uncorrelated_sell_gain_dayt_cutoff.csv")
write.csv(uncorrelated_sell_loss, file = "uncorrelated_sell_loss_dayt_cutoff.csv")